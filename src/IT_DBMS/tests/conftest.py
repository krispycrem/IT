...
from src.openapi_server.database.crud.table import save_table_metadata
from src.openapi_server.database.sql_alchemy import BaseORMModel
from src.openapi_server.main import app as application

from src.openapi_server.apis.utils import get_meta_db
...
@pytest.fixture()
def row1():
    return [1, 2.3, (1, 2), "some_stuff", 345]

@pytest.fixture()
def row2():
    return [2, 2.3, (1, 2), "some_stuff", 345]
...
def test_table_schema():
    table_schema = TableSchema(
        ...
        columns={
            ...
            "fifth": ColumnType.COLOR,
            "sixth": ColumnType.COLOR_INTERVAL,
        }
    )
    return table_schema
...
@pytest.fixture()
def mock_database_with_filled_duplicate_table_connection(
        mock_database_with_table_connection,
        test_database_name,
        test_table_schema
):
    mock_database_with_table_connection.execute(
        f"INSERT INTO {test_table_schema.table_name} "
        f"(first, second, third, fourth, fifth, sixth)"
        f"values (1, 1.2, 's', 'str', '_color', null),"
        f"(1, 1.2, 's', 'str', '_color', null);"
    )
...
def client(app) -> TestClient:
    from src.openapi_server.database.models.database import Database
    from src.openapi_server.database.models.table import Table
    from src.openapi_server.database.models.column import TableColumn
